...
# Updated motor angle (1번 -> 0도, 2번 -> 90도, 3번 -> 180도)


servo_min_duty = 3  # Set the minimum duty cycle to 3
servo_max_duty = 12  # Set the maximum duty cycle to 12
servo = GPIO.PWM(setting.MOTOR, 50)


def initialize_gpio():
    GPIO.cleanup()  # Initialize all GPIO ports
    GPIO.setmode(GPIO.BCM)  # Set GPIO pin numbering mode
    GPIO.setup(setting.SENSOR_PINS, GPIO.IN)  # Set SENSOR pins as input
    GPIO.setup(setting.PUMP, GPIO.OUT)  # Set PUMP pin as output
    GPIO.setup(setting.MOTOR, GPIO.OUT)  # Set MOTOR pin as output
...
def send_to_supabase(sensor_num: int, temp: float, humi: float):
    url = f"{setting.API_URL}/rest/v1/{setting.TABLE[sensor_num]}"
    headers = {
...
def check_sensor_conditions():
    triggered_sensors = []
    for i, pin in enumerate(setting.SENSOR_PINS):
        try:
            dht_device = setting.DHT_SENSORS[pin]
...
def motor_angle(sensor_list):
    ...
    for sensor_number in sorted(sensor_list):
        target_angle = setting.ANGLE[sensor_number]  # Get the target angle corresponding to the sensor number
...
try:
    while True:
        ...
        if result:
            motor_angle(result)
            GPIO.output(setting.PUMP, GPIO.HIGH)  # Turn pump on if condition is satisfied
            safe_print(f"Sensor {result} satisfied the condition. Pump is ON.")
            time.sleep(3)  # Keep the pump on for 3 seconds
            GPIO.output(setting.PUMP, GPIO.LOW)  # Turn pump off
